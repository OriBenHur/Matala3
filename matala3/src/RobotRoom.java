...
public class RobotRoom extends Canvas {
	...
	private static class Tile {
		...
		public static final Color wallColor = new Color( (float)0.1, (float)0.1, (float)0.1 );
	...
	public RobotRoom( String description ) {
		...
		window.addWindowListener(new WindowAdapter() {
			...
			public void windowDeiconified(WindowEvent e) {
				if(Info.getObj().getState() == Frame.ICONIFIED) Info.getObj().setState(Frame.NORMAL);
			}
			@Override
			public void windowIconified(WindowEvent e) {
				Info.getObj().setState(Frame.ICONIFIED);
			}
		});
		window.addComponentListener(new ComponentAdapter() {
			public void componentMoved(ComponentEvent e) {
				UpdateRobots();
				if(!Info.getObj().isVisible())Info.getObj().setVisible(true);
			}
		});
		...
		btnTurnLeft.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				robots.get(value).turnLeft();
				UpdateRobots();
				if(!Info.getObj().isVisible())Info.getObj().setVisible(true);
			}
		});

		JButton btnClick = new JButton("Pick Robot to Move");
		btnClick.setAlignmentX(Component.CENTER_ALIGNMENT);

		JButton btnTurnRight = new JButton("Turn Right");
		btnTurnRight.setAlignmentX(Component.RIGHT_ALIGNMENT);
		btnTurnRight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				robots.get(value).turnRight();
				UpdateRobots();
				if(!Info.getObj().isVisible())Info.getObj().setVisible(true);			
			}
		});
		...
		btnClick.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(robots.get(value).okToMove()){
					robots.get(value).move();
					UpdateRobots();
					if(!Info.getObj().isVisible())Info.getObj().setVisible(true);
				}
				else JOptionPane.showMessageDialog(null,"Robot "+value + " Can't Move to This Direction!", "Can't Move!", JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		while ( tokens.hasMoreTokens() ) {
			...
			try {
				...
				if (  row > 0  &&  row < height-1  &&  col > 0  &&  col < width-1  ) {
					...
					case 'g':
					case 'G':
						tiles[row][col].color = Color.gray;
						tiles[row][col].isWall = false;
						break;
					case 'b':
					case 'B':
		...
		int windowWidth = borders.left + borders.right + 2 * inset + width * ( Tile.WIDTH + 1 ) + 30;
		int windowHeight = borders.top + borders.bottom + 2 * inset + height * (Tile.HEIGHT + 1 ) + 35;
	...
	public static void UpdateRobots()
	{
		...
		{
			s=s+"Robot "+robots.get(i).getIndex() + " is in: Row: " + robots.get(i).getRow() + ", Col: "+ robots.get(i).getCol()+System.lineSeparator();
		}

		Info.getObj().setBounds(window.getX()+window.getWidth()-5,window.getY(),330, window.getHeight());
		Info.getObj().setTextPane(s);
		Info.getObj().setResizable(false);
		Info.getObj().setDefaultCloseOperation(0);
		//Info.getObj().dispose();
		//Info.getObj().setUndecorated(true);
	}
	...
	//		{
	//			s="/r/n "+robots[i].getIndex() + "Row: " + robots[i].getRow() + "Col: "+ robots[i].getCol();
	//		}
	//		return s;
	//
	//	}
	...
	protected boolean isObstructed( int col, int row ) {
		return (  tiles[row][col].isWall  ||  tiles[row][col].occupant != null || tiles[row][col].color.equals(Color.black));
	}
